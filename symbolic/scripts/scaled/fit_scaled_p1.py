...
def main():
    ...
    # Initialise
    itf = Interface("scaled_ox", input_path="../data", output_path="../results")
    ...
    for data in DATA_LIST:
        ...
        itf.add_data(data["path"], fitting=True, weight=1.0 if data["fit"] else 0.05)
    ...
    # Fit the model
    itf.fit_model()
    # julia = "((x0 * 0.16486052) * (1.4778193 ^ (x0 ^ (x1 * 18.969158)))) / (0.5041899 ^ (x1 ^ (x0 / x1)))"
    # julia = "(x0 * 0.16483812) * (exp((x1 ^ (x0 / x1)) / 1.4599572) / (0.6767317 ^ ((x0 ^ 18.94465) ^ x1)))"
    # julia = "((x1 * 0.33822554) ^ (x1 ^ ((x2 ^ x0) + -0.7338511))) * (x0 / (0.68963057 ^ (x0 ^ (x1 * 21.672562))))"
    # julia = "((((x0 ^ 42.766354) + 3.1716456) ^ x1) * 0.18641405) * ((x0 * x2) ^ (x0 + 0.9145794))"
    # julia = "(x0 ^ exp((x0 / x2) ^ exp((x0 ^ 5.5573115) + 0.4018126))) * (exp(-1.387401 / x1) + 0.2369791)"
    # itf.run_model_function("set_julia", julia=julia)
