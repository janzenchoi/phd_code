...
# Constants
EXPRESSION_LIST = [
    # "((exp(x0 * ((exp(x0) ^ -1.5063181) / x1)) + -0.7944972) * (-1.1079632 + (x1 + x2))) * ((x0 ^ 3.1733594) + 0.33957976)",
    # "((x0 ^ exp(((x0 ^ -1.9449484) ^ log(x0)) * 0.8161545)) * log((x2 ^ 1.8776461) * ((x1 / 0.57592446) + 0.9298026))) * x2"
    "((((x0 * -9.012826e-9) / log(x1 / 0.23705816)) ^ 1.6540794) ^ 1.003647) + 0.010980433"
]
CAL_ALL = True
DATA_LIST = [
...
def main(expression:str):
    ...
    * `expression`: The expression to evaluate
    ...
    # Initialise
    itf = Interface(f"eval_all", input_path="../data", output_path="../results", verbose=False)
    itf.define_model("eval_strain")
    ...
    for data in DATA_LIST:
        ...
        # itf.remove_after("time", data["tts"])
        # itf.remove_before("time", data["tts"])
        if data["ox"] != None:
        ...
        # # Scale time
        # def scale_time(t_list):
        #     t_max = max(t_list) if data["ox"] == None else data["ox"]
        #     # t_max = max(t_list)
        #     return [t/t_max for t in t_list]
        # itf.change_field("time", scale_time)
        
    # Fit the model
    itf.run_model_function("set_julia", julia=expression)
