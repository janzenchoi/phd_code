...
def main():
    ...
    # Initialise
    itf = Interface(f"tertiary", input_path="../data", output_path="../results")
    ...
    for data in DATA_LIST:
        ...
        itf.add_data(data["path"], fitting=data["fit"], weights=[0.5,1,2,4,8])
        itf.remove_before("time", data["tts"])
        if data["ox"] != None:
            itf.remove_after("time", data["ox"])
        itf.change_field("time", lambda t_list : [t-t_list[0] for t in t_list])
        itf.change_field("strain", lambda s_list : [s-s_list[0] for s in s_list])
