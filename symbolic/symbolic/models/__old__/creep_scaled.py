...
class Model(__Model__):
    ...
    def initialise(self, ttf_julia:str, **settings):
        """
        Initialises the model

        Parameters:
        * `ttf_julia`: The time-to-failure julia expression
        """
        self.ttf_julia = ttf_julia
    ...
    def predict(self, data_list:list) -> list:
        ...
        * `data_list`: List of dictionaries containing data
        ...
        for data in data_list:
            ...
            # Predict strain
            time_list = list(np.linspace(1e-5,1,128))
            ...
            # ttf = get_ttf(self.ttf_julia, temperature, stress)
...
def get_ttf(ttf_julia:str, temperature:float, stress:float) -> float:
    """
    Calculates the time-to-failure
    ...
    * `stress`:      The stress
    ...
    Returns the calculated time-to-failure
    """
    ttf_julia = process_str(ttf_julia)
    ttf_expression = {"f": ttf_julia}
    ttf = evaluate_expression(ttf_expression, "f", {"x0": [stress], "x1": [temperature]})[0]
    return ttf
