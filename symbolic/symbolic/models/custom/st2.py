...
class Model(__Model__):
    ...
    def initialise(self, **settings):
        ...
        # Define combine string for strain
        # x0 = time
        # x1 = stress
        # x2 = temperature
        # y0 = strain
        self.strain_combine = """
            A = 10^(p[1]); q = 10^(p[2]); m = abs(p[3]); n = abs(p[4]);
            zA = A * x0^m * x1^n * exp(-q/8.3145/x2);
            z0 = f0(x0,x1,x2,zA);
            re0 = abs((y0-z0)/y0);
            ref = re0;
            ref
        ...
        parameter_values = [-15.017, 3.6948, 5.054, 0.77361] # 800C
        parameter_map = dict(zip([f"p[{i+1}]" for i in range(len(parameter_values))], parameter_values))
        self.strain_combine = set_parameters(self.strain_combine, parameter_map)
    ...
    def fit(self, data_list:list) -> None:
        ...
        * `data_list`: List of dictionaries containing data
        ...
        self.julia = self.strain_reg.get_best()["julia_expression"]
        self.expression = julia_to_expression(self.julia, self.strain_combine)
    ...
    def get_latex(self) -> str:
        ...
        latex_expressions = [latex_dict[p] for p in ["z0", "zA", "A", "m", "n", "q"]]
