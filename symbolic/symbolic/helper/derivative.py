...
def differentiate_curve(data_dict:dict, x_label:str, y_label:str) -> dict:
    """
    ...
    * `y_label`:   The label of the y axis
    ...
    interpolator = Interpolator(data_dict[x_label], data_dict[y_label])
    interpolator.differentiate()
    data_dict[y_label] = interpolator.evaluate(data_dict[x_label])
    return data_dict

def integrate_curve(data_dict:dict, x_label:str, y_label:str) -> dict:
    """
    For integrating a curve
    ...
    * `y_label`:   The label of the y axis
    ...
    """
    data_dict = deepcopy(data_dict)
    x_list = np.array(data_dict[x_label])
    y_list = np.array(data_dict[y_label])
    data_dict[y_label] = list(cumtrapz(y_list, x_list, initial=0))
    return data_dict
