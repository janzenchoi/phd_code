...
def loss_function(params):
    ...
    if c >= a*b:
        return 1e5
    loss = np.average([abs(run_exp_linear(awr_list[i], a, b, c) - cw_list[i]) for i in range(len(awr_list))])
    return loss

result = differential_evolution(loss_function, bounds=[(0, 1e4), (0, 1e2), (0, 1e4)])
p = result.x
a, b, c = p[0], p[1], p[2]
