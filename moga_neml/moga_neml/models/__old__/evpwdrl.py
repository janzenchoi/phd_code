"""
 Title:         The Elastic Viscoplastic Work Damage Model with reciprocal-linear function for work damage
...
class Model(__Model__):
    ...
    def initialise(self):
        ...
        self.add_param("wd_a", 0.0e0, 1.0e4)
        self.add_param("wd_b", 0.0e0, 1.0e3)
        self.add_param("wd_c", 0.0e0, 1.0e4)
    ...
    def calibrate_model(self, evp_s0, evp_R, evp_d, evp_n, evp_eta, wd_a, wd_b, wd_c, wd_n0, wd_n1):
        ...
        * `...`: ...
        ...
        n_list  = [get_n(wr, wd_a, wd_b, wd_c, wd_n0, wd_n1) for wr in wr_list]
...
def get_n(wr:float, a:float, b:float, c:float, n_0:float, n_1:float) -> float:
    wr_0, wr_2 = WORK_RATE_BOUNDS
    wr_1 = get_intercept(a, b, c)
    if wr_0 < wr and wr <= wr_1:
        return n_0
