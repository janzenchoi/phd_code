...
class Simulation(__Simulation__):
    ...
    def post_process(self, sim_path:str, results_path:str, grain_map_path:str=None, exodus_prefix:str="") -> None:
        ...
        * `exodus_prefix`:  The prefix for the exodus files;
        ...
        grain_ids     = get_block_ids(sim_dict_list[-1], "block_id")
        grain_field_map = map_field(sim_dict_list[-1], "block_id", "id", grain_ids)

        # Calculate average stresses and elastic strains
        average_dict = {
            "average_strain":  results_dict["mTE_xx"],
            "average_stress":  results_dict["mCS_xx"],
            "average_elastic": results_dict["mEE_xx"],
        }
