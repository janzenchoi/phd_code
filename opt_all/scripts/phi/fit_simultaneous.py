...
# Initialise model
itf = Interface(f"phi_simultaneous", input_path="../data", output_path="../results")
itf.define_model("phi/simultaneous")
...
for i in [1,2,3,4]:
    ...
    itf.bind_param(f"a{i}", 0, 1e3)
    itf.bind_param(f"n{i}", 0, 1e1)
    itf.bind_param(f"q{i}", 0, 1e3)
    # itf.bind_param(f"a{i}", 0, 1e3)
...
for i, data in enumerate(DATA_LIST):
    ...
    # Add error
    # itf.add_error("area", labels=["time", "strain"], group=f"area_{i}", weight=1.0 if data["fit"] else 0.1)
    if data["fit"]:
