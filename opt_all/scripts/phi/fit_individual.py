...
for data in DATA_LIST:
    ...
    itf = Interface(f"phi_{data_info[1]}_{data_info[2]}", input_path="../data", output_path="../results")
    itf.define_model("phi/individual")

    # Bind parameters
    itf.bind_param(f"phi_1", 0, 10)
    itf.bind_param(f"phi_2", 0, 100)
    itf.bind_param(f"phi_3", 0, 10)
    itf.bind_param(f"phi_4", 0, 10)
