...
for _ in range(iters):
    ...
    # OUTPUT_PATH = f"../results/{TIMESTAMP}_theta_mt"
    # safe_mkdir(OUTPUT_PATH)

    OUTPUT_PATH = [
        "../results/2025-10-06 theta_mt/251006171837_theta_mt",
        "../results/2025-10-06 theta_mt/251006171855_theta_mt",
        "../results/2025-10-06 theta_mt/251006171858_theta_mt",
        "../results/2025-10-06 theta_mt/251006171901_theta_mt",
        "../results/2025-10-06 theta_mt/251006171906_theta_mt",
    ][int(sys.argv[2])]
    ...
    #     itf = Interface(f"theta_mt_{data_info[1]}_{data_info[2]}", input_path="../data", output_path=OUTPUT_PATH)
    #     itf.define_model("theta_mt/individual")

    #     # Bind parameters
    #     itf.bind_param(f"t1", -10, 1)
    #     itf.bind_param(f"t2",  0, 10)
    #     itf.bind_param(f"t3", -10, 1)
    #     itf.bind_param(f"t4",  0, 10)
    #     itf.bind_param(f"t5", -10, 1)
    ...
    for dir_path, data in zip(dir_path_list, DATA_LIST):
        ...
        if data["fit"]:
            cal_dict["temperature"].append(int(data_info[3])/1000)
            cal_dict["stress"].append(int(data_info[4])/80)
        else:
            val_dict["temperature"].append(int(data_info[3])/1000)
            val_dict["stress"].append(int(data_info[4])/80)
    ...
    for pn in PARAM_NAMES:
        ...
        # Bind parameters
        if pn == "t5":
            itf.define_model("theta_mt/param_2", theta_param=pn)
            itf.bind_param("a", -1e2, 1e2)
            itf.bind_param("n", -1e1, 1e1)
            itf.bind_param("q", -1e2, 1e2)
        else:
            itf.define_model("theta_mt/param", theta_param=pn)
            itf.bind_param("a", -1e2, 1e2)
