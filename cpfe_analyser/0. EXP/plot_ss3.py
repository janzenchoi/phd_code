...
def main():
    ...
    for unl_strain, unl_stress in zip(unl_strain_list, unl_stress_list):
        x_list = [unl_strain, unl_strain]
        y_list = [0, unl_stress]
        plt.plot(x_list, y_list, color=UNLOAD_COLOUR, linestyle="--", linewidth=1, zorder=3)
    plt.scatter(unl_strain_list, unl_stress_list, color=UNLOAD_COLOUR, marker="o", edgecolor=UNLOAD_COLOUR, s=6**2, zorder=3)
    handles = [
        plt.scatter([], [], color=INL_COLOUR,    label="Standard Specimen", s=8**2),
        plt.scatter([], [], color=TRU_COLOUR,    label="Miniaturised Specimen", s=6**2),
        plt.plot([], [], color=UNLOAD_COLOUR, label="EBSD Data Acquisition", linestyle="--", linewidth=1)[0],
    ]
